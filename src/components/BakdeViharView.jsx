...
import { ArrowLeft, Home, MessageCircle, Share2, Compass, Layers, Maximize2 } from "lucide-react";
...
function injectColonyStyles() {
  ...
  style.textContent = `
    ...
    .cm-brand{z-index:2;position:absolute;left:14px;top:calc(12px + env(safe-area-inset-top,0px));display:inline-flex;align-items:center;gap:12px;pointer-events:none;max-width:calc(100% - 140px);background:var(--panel);border:1px solid var(--line);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);padding:6px 14px 6px 8px;border-radius:16px;box-shadow:0 6px 18px rgba(0,0,0,.35)}
    ...
    .cm-infoCard{position:absolute;right:12px;top:calc(70px + env(safe-area-inset-top,0px));width:220px;padding:14px;border-radius:16px;background:rgba(15,23,42,0.95);backdrop-filter:blur(12px);box-shadow:0 16px 45px rgba(0,0,0,.6),0 0 0 1px rgba(255,255,255,0.08);z-index:4}
    ...
    .cm-panelHead button{border:0;background:rgba(255,255,255,.08);width:30px;height:30px;border-radius:999px;display:grid;place-items:center}
...
export default function BakdeViharView({ onNavigateAadhunikNaksha, onNavigateHome }) {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#0F172A] flex flex-col font-sans select-none">
      {/* Top Application Bar */}
      <header className="relative z-30 flex h-14 w-full items-center justify-between border-b border-white/10 bg-[#0B1120]/90 px-4 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-sm font-extrabold tracking-wide text-white">BAKDE VIHAR</span>
            <span className="ml-2 hidden rounded-full border border-[#FF7A00]/30 bg-[#FF7A00]/10 px-2 py-0.5 text-[10px] font-bold text-[#FF7A00] sm:inline-block">
              3D Master Plan
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          <a
            href="https://wa.me/916265597717?text=Hi%20Ar.%20Aman%20Verma,%20I%20am%20exploring%20the%20Bakde%20Vihar%203D%20layout%20and%20would%20like%20more%20information."
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs font-semibold text-emerald-400 hover:bg-emerald-500 hover:text-white transition-colors"
          >
          ...
          {onNavigateHome && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-xs font-medium text-white/70 hover:bg-white/10 hover:text-white"
            >
      ...
      {/* 3D Map Canvas Stage */}
      <div className="cm-app flex-1 overflow-hidden relative">
        <main className="cm-stage" id="stage" ref={stageRef}>
          ...
          <div className="cm-brand">
            ...
            <div>
              <span className="cm-brandName">BAKDE VIHAR</span>
          ...
          <div className="cm-planPanel" id="planPanel" hidden>
            <div className="cm-panelHead">
              <b>Bakde Vihar — Sanctioned Layout Drawing</b>
              <button id="closePlan" type="button" aria-label="Close">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 6 6 18" /><path d="m6 6 12 12" /></svg>
              </button>
            </div>
            <img src="/layout-plan.jpg" alt="Bakde Vihar colony master plan drawing" />
          </div>

          <div className="cm-infoPanel" id="infoPanel" hidden>
            <div className="cm-panelHead">
              <b>Bakde Vihar Project Info</b>
